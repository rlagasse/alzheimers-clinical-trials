...
-- Validate all 8 necessary tables exist
...
-- studies table PK is nct_id, all other tables have id as PK but also have nct_id to properly link to studies
...
-- Validate nct_id column exists in all but studies table; the foreign key to link to studies table
...
-- Validate alzheimer_subset preserves the same 8 tables and all columns from original schema ctgov
-- Compare ctgov and alzheimer_subset columns
SELECT set_eq(
	$$
	SELECT column_name
